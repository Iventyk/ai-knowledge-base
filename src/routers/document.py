...
async def upload_document(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)) -> DocumentCreateResponse:
...
async def delete_document(document_id: UUID, db: AsyncSession = Depends(get_db)) -> Response:

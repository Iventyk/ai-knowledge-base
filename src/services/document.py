...
class DocumentService:
    ...
    async def upload_document(self, file: UploadFile):
        ...
        if suffix not in ALLOWED_SUFFIXES:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Supported file types: txt, pdf, md")

        name, file_path = await self.storage.save(file)
        document = await self.repository.create(name=name, file_path=file_path)
        logger.info("document_created", document_id=str(document.id), filename=document.name)
    ...
    async def list_documents(self) -> list[DocumentListItem]:
        ...
        return [DocumentListItem.from_model(document) for document in documents]
    ...
    async def delete_document(self, document_id: UUID) -> None:
        ...
        logger.info("document_deleted", document_id=str(document.id), filename=document.name)
